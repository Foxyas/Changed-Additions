    ...
    public @NotNull ItemStack finishUsingItem(@NotNull ItemStack stack, @NotNull Level level, @NotNull LivingEntity entity) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ProcessTransfur.ifPlayerTransfurred(serverPlayer, (transfurVariantInstance) -> {
                ParticlesUtil.sendParticles(serverPlayer.getLevel(), ChangedParticles.drippingLatex(transfurVariantInstance.getTransfurColor()), serverPlayer.getEyePosition(), 0.25f, 0.25f, 0.25f, 10, 0);
            });
